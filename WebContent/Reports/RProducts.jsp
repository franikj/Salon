...
{
conn = DriverManager.getConnection("jdbc:sqlserver://VENERA;instanceName=MOON;databaseName=Salon;integratedSecurity=true");
String query = "EXEC ProductReport @datefrom=?, @dateto=?" ;
...
rss = ps.executeQuery();
   out.println("<div style='background-color: white'>");
	out.println("<center><table BORDER=1 CELLPADDING=0 CELLSPACING=0 WIDTH=70% >"
	        +"<tr><th><center>Ідентифікатор</center></th><th><center>Категорія</center></th><th><center>Товар</center></th><th><center>Кількість</center></th></tr>");
	...
	out.println("<tr><td><center>"+rss.getString("ID")+"</center></td>"
			...
			+ "<td><center>"+rss.getString("Product")+"</center></td>"
	         + "<td><center>"+rss.getString("Amount")+"</center></td>"
	        );
	}
